...
- **SC-016**: Structured IAM validation proves that exactly the reviewed platform handler statement includes both active-schema `backup-storage` actions with `Effect: Allow` and `Resource: '*'`, rejecting missing, wildcarded, relocated, or resource-scoped variants.
