...
- **FR-041**: The pinned IDP publisher's direct Python child tools MUST be installed in its managed Python 3.12 environment at the reviewed versions recorded in `vendor/idp.lock.json`: `cfn-lint` 1.53.0, Ruff 0.15.21, and uv 0.9.6. Their versions and executable presence MUST be part of the local cache identity and MUST pass dependency-metadata plus native-command smoke checks before the cache marker is written; repair MUST force-reinstall the exact tool and bridge packages without dependency drift so missing launchers are recreated, and deployment MUST NOT rely on a mutable global Ruff, cfn-lint, or uv installation.
- **FR-042**: A supported operator environment-configuration procedure MUST be a repository-owned PowerShell 7.2-or-later script that accepts repository-relative/parameterized inputs, obtains Azure and AWS identity only from authenticated CLI sessions, auto-selects the sole public Route 53 zone or requires one exact public-zone ID when several exist, and never prints cloud identifiers, profile names, contacts, host values, or the resulting configuration. It MUST reject tracked/example targets, authenticated-identity drift, private or ambiguous zones, invalid/distinctness-breaking hostnames, unsafe CA bundles, and malformed contacts; validate a UTF-8-no-BOM sibling candidate with the canonical environment reader; and atomically replace only an ignored environment file while keeping certificates and populated values outside source control.
...
- **SC-027**: Repository and deployment-helper tests reject publisher build-tool version drift, omission, missing executables, or cache identities that omit the tool pins; an offline package regression deletes a console launcher while retaining package metadata and proves force-reinstall recreates it, the reviewed Ruff version passes both `check` and `format --check` against the exact IDP 0.5.16 source, and cfn-lint/Ruff/uv native version checks complete before the local cache marker.
- **SC-028**: PowerShell and repository-mutation tests prove the environment configurator canonicalizes public DNS inputs, rejects unsafe contacts and trust bundles, preserves the original bytes after candidate-validation failure, emits UTF-8 without a byte-order mark, uses safe Azure/AWS argument boundaries, and redacts every synthetic tenant/subscription/account/profile/domain/contact value; a hosted Linux synthetic CLI run proves a successful rerun preserves the generated registry name and configuration identity.
